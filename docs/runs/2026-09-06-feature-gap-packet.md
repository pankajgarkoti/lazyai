...
Implemented on `feat/gap-packet` (2026-09-06); PR #2 to `main`: https://github.com/pankajgarkoti/lazyai/pull/2. Evidence: `go test -race -count=1 ./...` (14 packages ok), `go vet ./...`, `gofmt -l .` clean, and `scripts/test-sessions-tmux.py --real-opencode` (23/23 checks, `ALL TMUX CHECKS PASSED`).
...
- [~] G7: A version tag can produce downloadable artifacts and checksums only after required gates pass. — `.github/workflows/release.yml` (tag/version match, vet, race, 4 archives, `SHA256SUMS`) and `ci.yml` are added and reviewed; version bumped to 0.2.0 (`TestVersionWithoutTerminal`). Not executed: no tag was pushed (release authority not granted in this run), and the workflow commit is held locally because the available GitHub tokens lack the `workflow` scope needed to push `.github/workflows/*`; `gh auth refresh -h github.com -s workflow` then `git push origin feat/gap-packet` completes it. The tmux drive stays a documented local gate.
